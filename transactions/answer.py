...
####### Adding exchange rate adjustment
exchange_schema = StructType([StructField('currency', StringType()),
                              StructField('rate', FloatType()),
                             ])

exchange_df = spark.read.csv("exchange_rates.csv",
                             schema=exchange_schema,
                             header=True)

with_exchange_rates = joint_df.join(
    exchange_df, joint_df['currency'] == exchange_df['currency'])

with_exchange_rates.show()

currency_adjusted_df = with_exchange_rates.withColumn(
    'adjusted_amount',
    with_exchange_rates.amount * with_exchange_rates.rate)

currency_adjusted_df.show()

final_df = \
    currency_adjusted_df.select(
        col('customer_id'),
        col('name'),
        col('date'),
        col('adjusted_amount'))
